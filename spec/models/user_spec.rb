...
RSpec.describe User, type: :model do
  ...
  it { is_expected.to allow_value('adrianorosaferreira@gmail.com').for(:email) }
  ...
  describe '#generate_authentication_token!' do
    ...
    it 'generates another auth token when the current auth token already has been taken' do
      allow(Devise).to receive(:friendly_token).and_return('abc123tokenxyz', 'abc123tokenxyz', 'abcXYZ123456')
